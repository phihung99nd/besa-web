<template>
  <div class="point-mall">
    ...
    <Search></Search>
    <cateNav></cateNav>
    <h3 class="promotion-decorate">{{this.$t('points product')}}</h3>
    ...
    <div class="goods-list">
      ...
      >
        ...
        <div class="goods-show-price">
          <span>
            <span class="seckill-price text-danger">{{item.points}}{{ $t('point') }}</span>
          </span>
        </div>
        <div class="goods-show-detail">
          <span>{{ item.goodsSku.name }}</span>
        </div>
        <div class="goods-show-num">
          {{ $t('Already have') }}<span>{{ item.commentNum || 0 }}</span>{{ $t('') }}
        </div>
