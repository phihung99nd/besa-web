...
<template>
  <div class="add-eval">
    <div class="title">
      <card :_Title="$t('Complaint')" :_Size="16"></card>
      <p>
    ...
    <div class="goods-eval">
      ...
      <div class="eval-con">
        <div>
          <span class="color999">{{$t('Title')}}：</span>
          ...
          <Input type="textarea" maxlength="500" show-word-limit :rows="4" :placeholder="$t('Complaint content')" v-model="form.content" />
        </div>
        <div style="display:flex;align-items:center;">
          ...
          <div class="describe">{{$t('Upload complaint file, up to 5')}}</div>
        </div>
      </div>
    </div>
    <Button type="primary" class="mt_10" :loading="loading" @click="save">{{ $t('Submit') }}</Button>
  ...
  methods: {
    ...
    save () { // 提交投诉信息
      ...
      handleComplain(params).then(res => {
        if (res.success) {
          this.$Message.success(this.$t('Complaint application has been submitted, thank you for your feedback'))
    ...
    handleBeforeUpload () { // 上传之前钩子函数
      ...
      if (!check) {
        this.$Notice.warning({
          title: this.$t('Upload file, up to 5')
        });
        return check;
      }
    }
