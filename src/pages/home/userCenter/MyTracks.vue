<template>
  <div class="track-wrapper">
    <!-- 卡片组件 -->
    <card :_Title="$t('Footprint')" :_Size="16"></card>
    <Button class="del-btn" @click="clearAll" type="primary">{{ $t('Delete all') }}</Button>
    <!-- 订单列表 -->
    ...
    <ul class="track-list" v-else>
      ...
      >
        ...
        <p>{{ item.price | unitPrice("₫",  "before") }}</p>
...
export default {
  ...
  methods: {
    ...
    clearAll() {
      ...
      this.$Modal.confirm({
        title: this.$t('Delete'),
        content: this.$t('Are you sure you want to delete all footprints'),
        onOk: () => {
