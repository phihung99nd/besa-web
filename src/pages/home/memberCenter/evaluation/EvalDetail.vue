<template>
  <div class="add-eval">
    <div class="title">
      <card :_Title="$t('Review Detail')" :_Size="16"></card>
      <p>
    ...
    <div class="delivery-rate">
      ...
      <div class="color999">
        <span>{{$t('Logistics rating')}}：<Rate disabled :value="Number(orderGoods.deliveryScore)" /></span>
        <span>{{$t('Service rating')}}：<Rate disabled :value="Number(orderGoods.serviceScore)" /></span>
        <span
          >{{$t('Service rating')}}：<Rate disabled :value="Number(orderGoods.descriptionScore)"
        /></span>
      </div>
    </div>
    <!-- 添加订单评价  左侧商品详情  右侧评价框 -->
        ...
        <div class="eval-con">
          <div>
            ...
            <RadioGroup
              ...
              button-style="solid"
            >
              <Radio label="GOOD" disabled>{{$t('Good')}}</Radio>
              <Radio label="MODERATE" disabled>{{$t('Moderate')}}</Radio>
              <Radio label="WORSE" disabled>{{$t('Worse')}}</Radio>
            </RadioGroup>
            <Input
              type="textarea"
              maxlength="500"
          ...
          <div>
            <span class="color999">{{$t('Seller')}}：</span>
