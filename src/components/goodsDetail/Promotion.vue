<template>
  <div>
    <div class="promotion-wrapper" v-if="type === 'goodsDetail'">
      <div class="wr-l" style="display: flex">
<!--        <Icon size="23" type="ios-alarm-outline" /> -->
      ...
      <p>{{$t('Countdown')}}：</p><span>{{ hours }}</span><span>{{ minutes }}</span><span>{{ seconds }}</span>
      </div>
      <div v-else>{{end}}</div>
    </div>
    <span v-else class="cart-promotion">
      <span v-if="end === ''">{{ $t('Event ends in') }}：<span>{{ hours }}</span> : <span>{{ minutes }}</span> : <span>{{ seconds }}</span></span>
      <span v-else>{{$t('The event is over')}}</span>
    </span>
  </div>
</template>
<script>
