                ...
                <div class="remarks-analyse-box">
                  <div class="remarks-analyse-goods">
                    <i-circle :percent="skuDetail.grade" stroke-color="#5cb85c">
                      ...
                      <p class="remarks-analyse-title">{{$t('Good')}}</p>
                    </i-circle>
                  </div>
                </div>
                <div class="remarks-bar">
                  <span @click="viewByGrade('')" :class="{selectedBar: commentParams.grade === ''}">{{ $t('All ({0})', [commentTypeNum.all]) }}</span>
                  <span @click="viewByGrade('GOOD')" :class="{selectedBar: commentParams.grade === 'GOOD'}">{{$t('Good')}} ({{commentTypeNum.good}})</span>
                  <span @click="viewByGrade('MODERATE')" :class="{selectedBar: commentParams.grade === 'MODERATE'}">{{$t('Moderate')}} ({{commentTypeNum.moderate}})</span>
                  <span @click="viewByGrade('WORSE')" :class="{selectedBar: commentParams.grade === 'WORSE'}">{{$t('Worse')}} ({{commentTypeNum.worse}})</span>
                </div>
                ...
                <div class="remarks-box" v-for="(item,index) in commentList" :key="index" v-else>
                  ...
                  <div class="remarks-content-box">
                    ...
                    <div class="preview-img"  v-if="item.previewImg"  @click.prevent="hidePreviewImg(item)">
                      <div>
                        <span @click.stop="rotatePreviewImg(0, item)"><Icon type="md-refresh" />{{ $t('Left') }}</span>
                        <span @click.stop="rotatePreviewImg(1, item)"><Icon type="md-refresh" />{{ $t('Right') }}</span>
                      </div>
                      <img :src="item.previewImg" :style="{transform:`rotate(${item.deg}deg)`}" width="198" alt="">
                    </div>
                    <p class="remarks-sub">
                      <span class="remarks-item">{{item.name}}</span>
